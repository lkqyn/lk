...
/* printf debug output enabled: do not mask printf */
...
static uint8_t AppDebug_ReadCommand(void)
{
  uint8_t data = 0;
...
static void AppDebug_PrintHelp(void)
{
  printf("Debug commands:\r\n");
  printf("  p/P: print normal label by UART5\r\n");
  printf("  t/T: print simple UART5 test label\r\n");
  printf("  s/S: query printer ~HS status by UART5\r\n");
  printf("  b/B: set printer serial baud to 115200\r\n");
  printf("  m/M: set max label length to 1890 dots and save\r\n");
  printf("  h/H: show this help\r\n");
}

static void AppDebug_PrintSimpleTest(void)
{
  static const char zpl[] =
    ...
    "^LL945\r\n"
    "^LH0,0\r\n"
    "^FO50,100^GB1300,8,8^FS\r\n"
    "^FO50,300^GB1300,8,8^FS\r\n"
    "^FO50,500^GB1300,8,8^FS\r\n"
    "^FO50,700^GB1300,8,8^FS\r\n"
    "^FO50,900^GB1300,8,8^FS\r\n"
    "^FO60,110^A0N,40,40^FDY100^FS\r\n"
    "^FO60,310^A0N,40,40^FDY300^FS\r\n"
    "^FO60,510^A0N,40,40^FDY500^FS\r\n"
    "^FO60,710^A0N,40,40^FDY700^FS\r\n"
    "^FO60,860^A0N,40,40^FDY900^FS\r\n"
    "^XZ\r\n";

  printf("Send UART5 coordinate test label\r\n");
  AppPrinter_SendZpl(zpl);
}

static void AppDebug_QueryPrinterStatus(void)
{
  uint8_t status[256];
  uint16_t len;

  printf("Query printer status by ~HS\r\n");
  len = AppPrinter_QueryHostStatus(status, sizeof(status), 1000);

  if (len == 0)
  {
    printf("No printer status received\r\n");
    return;
  }

  printf("Printer status received, len=%u\r\n", len);
  printf("%s\r\n", status);
}

static void AppDebug_SetMaxLabelLength(void)
{
  static const char zpl[] =
    "^XA\r\n"
    "^ML1890\r\n"
  ...
  printf("Set printer max label length to 1890 dots and save\r\n");
  AppPrinter_SendZpl(zpl);
}
...
void AppTask(void *argument)
{
  ...
  AppDebug_PrintHelp();
  printf("Configure printer serial baud to 115200\r\n");
  AppPrinter_InitBaud115200();
  ...
  for (;;)
  {
    ...
    debugCommand = AppDebug_ReadCommand();

    if (debugCommand == 'h' || debugCommand == 'H')
    {
      AppDebug_PrintHelp();
    }
    else if (debugCommand == 'b' || debugCommand == 'B')
    {
      printf("Set printer serial baud to 115200\r\n");
      AppPrinter_InitBaud115200();
    }
    else if (debugCommand == 's' || debugCommand == 'S')
    {
      AppDebug_QueryPrinterStatus();
    }
    else if (debugCommand == 'm' || debugCommand == 'M')
    {
      AppDebug_SetMaxLabelLength();
    }
    else if (debugCommand == 't' || debugCommand == 'T')
    {
      AppDebug_PrintSimpleTest();
    }
    else if (printState == PRINT_STATE_IDLE && (debugCommand == 'p' || debugCommand == 'P'))
    {
      manualPrintActive = 1;
      printTrigger = 1;
