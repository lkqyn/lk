...
void REIN_mt6816_spi_data_Signal_Init(void)   // 编码器校准，识别编码器方向，非线性校准等
{
    ...
    float bbb = 48.0f / vol;
    if(bbb > 4.0f) bbb = 4.0f;
    else if(bbb < 0.5f) bbb = 0.5f;
    ...
    for(uint16_t i = 0; i < 200; i++)
    {
        ...
        {
            foc_open(&m1_foc, (uint16_t)(j + (i % 4) * 256));
            SysTick_Delay_us(60);
        }
        //HAL_IWDG_Refresh(&hiwdg);   // 看门狗清零
    }
    ...
    for(uint16_t i = 0; i < 200; i++)
    {
        ...
        for(uint16_t j = 0; j < 256; j++)
        {
            foc_open(&m1_foc, (uint16_t)(1023 - j - 256 * (i % 4)));
            SysTick_Delay_us(60);
        }
        //HAL_IWDG_Refresh(&hiwdg);   // 看门狗清零
    }
...
bool RINE_mt6816_spi_data_Get_AngleData(void)
{
    ...
    for(uint8_t i = 0; i < 3; i++)
    {
        // 读取SPI数据
        MT6816_SPI_CS_L();
        if(HAL_SPI_TransmitReceive(&MT6816_SPI_Get_HSPI, (uint8_t*)&data_t[0], (uint8_t*)&data_r[0], 1, 5) != HAL_OK)
        {
            MT6816_SPI_CS_H();
            mt6816_spi_data.parity_ok = false;
            return false;
        ...
        MT6816_SPI_CS_L();
        if(HAL_SPI_TransmitReceive(&MT6816_SPI_Get_HSPI, (uint8_t*)&data_t[1], (uint8_t*)&data_r[1], 1, 5) != HAL_OK)
        {
            MT6816_SPI_CS_H();
            mt6816_spi_data.parity_ok = false;
            return false;
