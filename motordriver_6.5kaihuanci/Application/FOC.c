...
void Sector_tracker(void)
{
    ...
    {
        m1_foc.angle = (int16_t)(DIR * (mt6816_count - angledata[m1_foc.angle_sector]) * 256 / m1_foc.scope);
    }
    else
    {
        if(DIR == 1)
        {
            if(mt6816_count > 16200)
                m1_foc.angle = (int16_t)((mt6816_count - angledata[m1_foc.angle_sector]) * 256 / m1_foc.scope);
            else
                m1_foc.angle = (int16_t)((16384 - angledata[m1_foc.angle_sector] + mt6816_count) * 256 / m1_foc.scope);
        }
        else
        {
            if(mt6816_count > 16200)
...
void can_foc_init(void)
{
    ...
    m1_foc.kp = 1.2f * PID_factor;
    m1_foc.ki = 0.010f * PID_factor;
    m1_foc.kd = 0.0f;
