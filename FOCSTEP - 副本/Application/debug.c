/*
 * USB CDC 命令与遥测实现。
 * 所有串口发送均在主循环推进；ADC、TIM 等实时中断不直接打印或阻塞。
 */
#include "debug.h"
...
#define DEBUG_VOFA_CHANNEL_COUNT   (9U)
#define DEBUG_VOFA_FRAME_SIZE      ((DEBUG_VOFA_CHANNEL_COUNT + 1U) * sizeof(float))
/* 14路数据加JustFloat帧尾为60字节，避免64字节整包额外的ZLP。 */
#define DEBUG_VOFA_SPEED_CHANNEL_COUNT (14U)
...
/* 15路数据加JustFloat帧尾为64字节，位置实时模式按1 kHz发送。 */
#define DEBUG_VOFA_POSITION_CHANNEL_COUNT (15U)
...
/*
 * 脉冲末端验证：14路数据加JustFloat帧尾为60字节，按1kHz发送。
 * 避免64字节整数包触发CDC额外的零长度包。
 * 6~10路报告STEP捕获中断快照；11路为遥测缺失计数，12~13路为到位测量。
 */
#define DEBUG_VOFA_PULSE_CHANNEL_COUNT (14U)
#define DEBUG_VOFA_PULSE_FRAME_SIZE \
    ((DEBUG_VOFA_PULSE_CHANNEL_COUNT + 1U) * sizeof(float))

#define DEBUG_PULSE_MODE_NAME "trajectory_three_loop"
...
/* 共享缓冲区容量独立于单个模式帧长，避免缩短脉冲帧后其他模式越界。 */
#define DEBUG_VOFA_TX_BUFFER_SIZE  (128U)
/* 兼容当前C编译器：任一发送帧超出容量时直接编译报错。 */
typedef char Debug_VofaFrameCapacityCheck[
    ((DEBUG_VOFA_FRAME_SIZE * DEBUG_VOFA_FRAMES_PER_USB_PACKET <= DEBUG_VOFA_TX_BUFFER_SIZE) &&
     (DEBUG_VOFA_SPEED_FRAME_SIZE <= DEBUG_VOFA_TX_BUFFER_SIZE) &&
     (DEBUG_VOFA_SPEED_BASIC_FRAME_SIZE <= DEBUG_VOFA_TX_BUFFER_SIZE) &&
     (DEBUG_VOFA_POSITION_FRAME_SIZE <= DEBUG_VOFA_TX_BUFFER_SIZE) &&
     (DEBUG_VOFA_POSITION_DIAGNOSTIC_FRAME_SIZE <= DEBUG_VOFA_TX_BUFFER_SIZE) &&
     (DEBUG_VOFA_POSITION_CAPTURE_FRAME_SIZE <= DEBUG_VOFA_TX_BUFFER_SIZE) &&
     (DEBUG_VOFA_PULSE_FRAME_SIZE <= DEBUG_VOFA_TX_BUFFER_SIZE)) ? 1 : -1];
...
typedef struct
{
    ...
    uint32_t position_last_sample_tick_ms;
    uint32_t position_dropped_samples;
    uint8_t pulse_live_mode;
    uint32_t pulse_sequence;
    uint32_t pulse_last_sample_tick_ms;
    uint32_t pulse_dropped_samples;
    uint32_t pulse_peak_bus_voltage_mv;
...
/* 以 JustFloat 约定的 4 字节浮点格式写入一个通道值。 */
    ...
    uint8_t position_mode)
{
    ...
    /*
     * 实时速度调试固定14路：速度目标/反馈/误差、Iq命令/反馈、
     * 速度PI P/I项、Id命令/反馈、Uq、限幅、弱磁和累计丢样数。
     * 该帧只在1kHz速度环样本到来时发出，内容是发送时刻的最新快照。
     */
    (void)position_mode;
    (void)sample;
    Debug_VofaWriteFloat(&destination[0U * sizeof(float)],
    ...
    Debug_VofaWriteFloat(&destination[8U * sizeof(float)],
                         (float)speed->current_loop_id_target_ma / 1000.0f);
    ...
    Debug_VofaWriteFloat(&destination[13U * sizeof(float)],
                         (float)SpeedLoop_GetTraceDroppedCount());
...
/*
 * 位置环JustFloat固定15路：最终目标、轨迹位置、轴位置、位置误差、
 * 速度/电流闭环状态与累计丢样数。位置单位为编码器计数；速度单位为rpm；
 * 电流单位为A。
 */
/* 组装位置、速度参考和编码器反馈的基础位置波形帧。 */
                                         ...
                                         uint32_t elapsed_ms)
{
    ...
    Debug_VofaWriteFloat(&destination[0U * sizeof(float)],
                         (float)elapsed_ms);
    Debug_VofaWriteFloat(&destination[1U * sizeof(float)],
                         (float)position->target_position_count);
    Debug_VofaWriteFloat(&destination[2U * sizeof(float)],
    ...
    Debug_VofaWriteFloat(&destination[12U * sizeof(float)],
                         (float)current->voltage_saturated);
    Debug_VofaWriteFloat(&destination[13U * sizeof(float)],
                         (float)position->reached);
    Debug_VofaWriteFloat(&destination[14U * sizeof(float)],
                         (float)s_vofa.position_dropped_samples);
...
/*
 * 脉冲模式专用末端验证帧，固定1kHz发送。
 * 通道0~5为当前状态，6~10为最近STEP捕获中断快照；停止发脉冲后保留
 * 最后一份快照。11为遥测缺失计数，12为首次到位耗时，13为曾到位标志。
 * STEP时间由硬件锁存，编码器位置在捕获ISR读取，存在中断响应延迟。
 */
/* 组装 STEP 输入、内部轨迹及末脉冲捕获信息的专用波形帧。 */
static void Debug_VofaBuildPulseFrame(uint8_t *destination)
{
    ...
    Debug_VofaWriteFloat(&destination[2U * sizeof(float)],
                         (float)pulse->reference_position_count);
    Debug_VofaWriteFloat(&destination[3U * sizeof(float)],
                         (float)position->reference_position_count);
    Debug_VofaWriteFloat(&destination[4U * sizeof(float)],
                         (float)position->measured_position_count);
    Debug_VofaWriteFloat(&destination[5U * sizeof(float)],
                         (float)(pulse->reference_position_count -
                                 position->measured_position_count));
    Debug_VofaWriteFloat(&destination[6U * sizeof(float)],
                         (float)pulse->last_edge_reference_position_count);
    Debug_VofaWriteFloat(&destination[7U * sizeof(float)],
                         (float)pulse->last_edge_command_position_count);
    Debug_VofaWriteFloat(&destination[8U * sizeof(float)],
                         (float)pulse->last_edge_measured_position_count);
    Debug_VofaWriteFloat(&destination[9U * sizeof(float)],
                         (float)(pulse->last_edge_reference_position_count -
                                 pulse->last_edge_measured_position_count));
    Debug_VofaWriteFloat(&destination[10U * sizeof(float)],
                         (float)(pulse->last_edge_reference_position_count -
                                 pulse->last_edge_command_position_count));
    Debug_VofaWriteFloat(&destination[11U * sizeof(float)],
                         (float)s_vofa.pulse_dropped_samples);
    Debug_VofaWriteFloat(&destination[12U * sizeof(float)],
                         (float)pulse->last_edge_reach_time_us / 1000.0f);
    Debug_VofaWriteFloat(&destination[13U * sizeof(float)],
                         (float)pulse->last_edge_reached);
...
static void Debug_VofaStop(void)
{
    ...
    s_vofa.position_last_sample_tick_ms = 0U;
    s_vofa.position_dropped_samples = 0U;
    s_vofa.pulse_live_mode = 0U;
    s_vofa.pulse_sequence = 0U;
    s_vofa.pulse_last_sample_tick_ms = 0U;
    s_vofa.pulse_dropped_samples = 0U;
    s_vofa.pulse_peak_bus_voltage_mv = 0U;
}
...
uint8_t Debug_StartPulseVofa(void)
{
    ...
    s_vofa.pulse_dropped_samples = 0U;
    s_vofa.pulse_peak_bus_voltage_mv = 0U;
    /* 首帧在USB CDC真正就绪后发送，避免未连接主机期间虚报丢样。 */
    s_vofa.pulse_last_sample_tick_ms = 0U;
    s_vofa.transfer_in_progress = 0U;
    s_vofa.active = 1U;
...
static void Debug_VofaLoop(void)
{
    ...
    if (s_vofa.pulse_live_mode != 0U)
    {
        ...
        if ((s_vofa.pulse_last_sample_tick_ms != 0U) &&
            ((now_ms - s_vofa.pulse_last_sample_tick_ms) > 1U))
        {
            s_vofa.pulse_dropped_samples +=
                (now_ms - s_vofa.pulse_last_sample_tick_ms - 1U);
        }
        /* 本毫秒已取样；发送失败不允许在同一毫秒内重复记数。 */
    ...
    if (s_vofa.position_live_mode != 0U)
    {
        ...
        now_ms = HAL_GetTick();
        if (now_ms == s_vofa.position_last_sample_tick_ms)
        {
            return;
        }

        if ((s_vofa.position_last_sample_tick_ms != 0U) &&
            ((now_ms - s_vofa.position_last_sample_tick_ms) > 1U))
        {
            s_vofa.position_dropped_samples +=
                (now_ms - s_vofa.position_last_sample_tick_ms - 1U);
        }
        /* 本毫秒已取样；本次USB提交失败也不能在同一毫秒重复累计。 */
        s_vofa.position_last_sample_tick_ms = now_ms;
        ...
        {
            Debug_VofaBuildPositionFrame(s_vofa.tx_buffer, now_ms);
        }
        if (CDC_Transmit_FS(s_vofa.tx_buffer,
                            (s_vofa.position_capture_mode != 0U) ?
    ...
    /* 不再支持“测试结束后回放”；VOFA 仅保留实时数据模式。 */
    Debug_VofaStop();
}

/* 将一条文本响应排队，经 USB CDC 异步发往主机。 */
static void Debug_SendText(const char *text)
{
    uint16_t length;

    /* JustFloat期间禁止混入ASCII回包；命令仍由解析器执行。 */
    if (s_vofa.active != 0U)
    {
        return;
    }
                                        ...
                                        uint8_t current_hold_enabled)
{
    ...
    /* 调试持续运行，由用户停机或故障保护退出。 */
    config.maximum_run_time_ms = UINT32_MAX;
    ...
    int32_t maximum_speed_mrpm)
{
    ...
    config.external_torque_kp_ma_per_count = 20L;
    config.external_torque_kd_ma_per_rpm = 8L;
    config.external_torque_limit_ma = 2200L;
    config.external_deceleration_kd_mrpm_per_rpm =
        s_position_tune.kd_mrpm_per_rpm;
    config.external_hold_kd_mrpm_per_rpm = s_position_tune.kd_mrpm_per_rpm;
...
/* 用外部板实际的 aimFreq 启动；接收端未知总数、减速段和最终边沿。 */
static uint8_t Debug_StartPulseGeneratorHz(uint32_t pulse_count,
                                           uint32_t aim_frequency_hz,
                                           int8_t direction)
{
    PulseGenerator_Config_t config;

    if (Debug_StartPulseInput() == 0U)
    {
        return 0U;
    }
    config.total_pulse_count = pulse_count;
    config.aim_frequency_hz = aim_frequency_hz;
    config.direction = direction;
    return PulseGenerator_Start(&config);
}

/* 将便利输入的目标 rpm 转为外部板 aimFreq 后启动。 */
static uint8_t Debug_StartPulseGenerator(uint32_t pulse_count,
                                         uint32_t aim_speed_rpm,
                                         int8_t direction)
{
    uint64_t aim_frequency_hz = ((uint64_t)aim_speed_rpm * 800ULL) / 60ULL;

    if (aim_frequency_hz > UINT32_MAX)
    {
        return 0U;
    }
    return Debug_StartPulseGeneratorHz(pulse_count,
                                       (uint32_t)aim_frequency_hz,
                                       direction);
}

/* 输出单板发生器状态；该状态不参与 PulseInput 的控制决策。 */
static void Debug_SendPulseGeneratorState(void)
{
    const volatile PulseGenerator_State_t *generator =
        PulseGenerator_GetState();

    (void)snprintf(s_debug_tx_buffer,
                   sizeof(s_debug_tx_buffer),
                   "SIM requested=%lu emitted=%lu aim=%luHz ramp=%u dir=%d running=%u completed=%u fault=%u late=%lu max_gap=%luus\r\n",
                   (unsigned long)generator->requested_pulse_count,
                   (unsigned long)generator->emitted_pulse_count,
                   (unsigned long)generator->aim_frequency_hz,
                   (unsigned int)generator->ramp_pulse_count,
                   (int)generator->direction,
                   (unsigned int)generator->running,
                   (unsigned int)generator->completed,
                   (unsigned int)generator->fault,
                   (unsigned long)generator->late_schedule_count,
                   (unsigned long)generator->maximum_injection_interval_us);
    Debug_SendText(s_debug_tx_buffer);
}

/* 输出脉冲累计位置、内部轨迹、末脉冲和到位捕获状态。 */
static void Debug_SendPulseInputState(void)
{
    ...
    (void)snprintf(s_debug_tx_buffer,
                   sizeof(s_debug_tx_buffer),
                   "PULSE mode=" DEBUG_PULSE_MODE_NAME " step_per_rev=800 accepted=%lu ignored=%lu ref=%ld "
                   "raw_speed=%ld ref_speed=%ld stream=%u edge=%lu/pulse=%ld/cmd=%ld/actual=%ld/err=%ld/traj=%ld/servo=%ld reach=%u/%luus stable=%u/%luus last_step=%lums/%ld/%ld/err=%ld "
                   ...
                   (long)pulse->last_edge_command_position_count,
                   ...
                   (long)(pulse->last_edge_command_position_count -
                          pulse->last_edge_measured_position_count),
                   (unsigned int)pulse->last_edge_reached,
                   (unsigned long)pulse->last_edge_reach_time_us,
                   (unsigned int)pulse->last_edge_stable,
                   (unsigned long)pulse->last_edge_stable_time_us,
...
/* 持续实时速度模式：首次启动，后续命令只在线改变目标、不断开 JustFloat。 */
static uint8_t Debug_StartOrSetVofaSpeedLive(int32_t target_speed_mrpm)
{
    if (s_vofa.speed_live_mode != 0U)
    {
        return SpeedLoop_SetTargetSpeedMrpm(target_speed_mrpm);
    }
    if (s_vofa.active != 0U)
    {
        return 0U;
    ...
    /* 单独调速度环时撤销自动脉冲位置链，避免两个外环竞争同一个速度环。 */
    PulseInput_Disable();
    PositionLoop_Stop();
    SpeedLoop_Stop();
    Debug_StartVofaSpeedTest(target_speed_mrpm, 0U, 0U, 0U);
    return (uint8_t)(s_vofa.speed_live_mode != 0U);
}
...
/* 持续输出实时 d/q 电流；同轴目标可在 JustFloat 输出中在线更新。 */
static uint8_t Debug_StartVofaDqLive(uint8_t q_axis, int32_t target_ma)
{
    if (MotorTest_IsDqLive() != 0U)
    {
        if (q_axis != MotorTest_IsDqLiveQAxis())
        {
            return 0U;
        }
        return MotorTest_SetDqLiveTarget(target_ma);
    }
    if (s_vofa.active != 0U)
    {
        return 0U;
    ...
    /* 电流环单独调试必须先撤销产品位置/速度链，避免双重控制同一 PWM。 */
    PulseInput_Disable();
    PositionLoop_Stop();
    SpeedLoop_Stop();
    Debug_VofaStop();
    if (MotorTest_StartDqLive((q_axis != 0U) ? 0L : target_ma,
                              (q_axis != 0U) ? target_ma : 0L) == 0U)
    {
        return 0U;
    }
...
/* 解析一条完整 ASCII 串口命令，并分发至测试/控制/查询处理。 */
static void Debug_ProcessCommand(const char *command)
{
    ...
    long simulated_pulse_count;
    long simulated_peak_speed_rpm;
    long simulated_direction;
    if ((s_vofa.active != 0U) &&
        !((s_vofa.pulse_live_mode != 0U) &&
          ((strncmp(command, "sim start ", 10U) == 0) ||
           (strncmp(command, "sim hz ", 7U) == 0) ||
           (strcmp(command, "sim stop") == 0))) &&
        (strcmp(command, "vofa stop") != 0) &&
        (strcmp(command, "pulse vofa off") != 0) &&
        (strncmp(command, "vofa current ", 13U) != 0) &&
        (strncmp(command, "vofa speed target ", 18U) != 0) &&
    ...
    if (strcmp(command, "help") == 0)
    {
        Debug_SendText("help | enc | zero | adc | adc zero | bus | test noise | test a+ | test a- | "
                       ...
                       "test iloop b- | "
                       "vofa current id MA | vofa current iq MA | vofa speed target RPM | vofa speed RPM | vofa speed step FROM TO | vofa speed loop RPM | "
                       ...
                       "sim start PULSE_COUNT AIM_RPM DIR | sim hz PULSE_COUNT AIM_HZ DIR | sim stop | sim status | "
    ...
    else if (sscanf(command, "sim hz %ld %ld %ld",
                    &simulated_pulse_count,
                    &simulated_peak_speed_rpm,
                    &simulated_direction) == 3)
    {
        if ((simulated_pulse_count <= 0L) ||
            (simulated_peak_speed_rpm < 600L) ||
            ((simulated_direction != 1L) && (simulated_direction != -1L)) ||
            (Debug_StartPulseGeneratorHz((uint32_t)simulated_pulse_count,
                                         (uint32_t)simulated_peak_speed_rpm,
                                         (int8_t)simulated_direction) == 0U))
        {
            Debug_SendText("err: sim hz requires valid PULSE_COUNT AIM_HZ DIR and idle generator\r\n");
        }
        else
        {
            Debug_SendText("ok: external-table SIM started; receiver is not told the final step\r\n");
            Debug_SendPulseGeneratorState();
        }
    }
    else if (sscanf(command, "sim start %ld %ld %ld",
                    &simulated_pulse_count,
                    &simulated_peak_speed_rpm,
                    &simulated_direction) == 3)
    {
        if ((simulated_pulse_count <= 0L) ||
            (simulated_peak_speed_rpm <= 0L) ||
            ((simulated_direction != 1L) && (simulated_direction != -1L)) ||
            (Debug_StartPulseGenerator((uint32_t)simulated_pulse_count,
                                       (uint32_t)simulated_peak_speed_rpm,
                                       (int8_t)simulated_direction) == 0U))
        {
            Debug_SendText("err: sim start requires valid PULSE_COUNT AIM_RPM DIR and idle generator\r\n");
        }
        else
        {
            Debug_SendText("ok: sim started; receiver is not told the final step\r\n");
            Debug_SendPulseGeneratorState();
        }
    }
    else if (strcmp(command, "sim stop") == 0)
    {
        PulseGenerator_Stop();
        Debug_SendText("ok: sim stopped without end notification\r\n");
        Debug_SendPulseGeneratorState();
    }
    else if (strcmp(command, "sim status") == 0)
    {
        Debug_SendPulseGeneratorState();
        Debug_SendPulseInputState();
    }
    ...
    else if (sscanf(command, "vofa current id %ld", &target_position_count) == 1)
    {
        if (((target_position_count == 0L) &&
             (MotorTest_IsDqLive() == 0U)) ||
            (target_position_count < -1000L) ||
            (target_position_count > 1000L) ||
            (Debug_StartVofaDqLive(0U, target_position_count) == 0U))
        {
            /* JustFloat 活跃时不插入文本，VOFA 波形不会被破坏。 */
            if (s_vofa.active == 0U)
            {
                Debug_SendText("err: vofa current id requires idle loop and nonzero -1000..1000mA\r\n");
            }
        }
    }
    else if (sscanf(command, "vofa current iq %ld", &target_position_count) == 1)
    {
        if (((target_position_count == 0L) &&
             (MotorTest_IsDqLive() == 0U)) ||
            (target_position_count < -1000L) ||
            (target_position_count > 1000L) ||
            (Debug_StartVofaDqLive(1U, target_position_count) == 0U))
        {
            /* Iq 可能产生转矩；超过保护范围或不满足启动条件时不执行。 */
            if (s_vofa.active == 0U)
            {
                Debug_SendText("err: vofa current iq requires idle loop and nonzero -1000..1000mA\r\n");
            }
        }
    }
    else if (sscanf(command, "vofa speed target %ld", &target_speed_rpm) == 1)
    {
        if ((target_speed_rpm < -DEBUG_SPEED_COMMAND_MAXIMUM_RPM) ||
            (target_speed_rpm > DEBUG_SPEED_COMMAND_MAXIMUM_RPM) ||
            (Debug_StartOrSetVofaSpeedLive(
                 (int32_t)target_speed_rpm * 1000L) == 0U))
        {
            /* JustFloat运行时不插入错误文本，避免破坏波形协议。 */
            if (s_vofa.active == 0U)
            {
                Debug_SendText("err: vofa speed target requires idle loop and valid RPM\r\n");
            }
        }
    }
    else if (sscanf(command,
...
/* 主循环调试服务：处理命令、文本队列、测试导出和 VOFA 帧。 */
void Debug_Loop(void)
{
    ...
    /* 模式切换可能复用tx_buffer；等待上一帧释放后再处理，停机不受此限制。 */
    if (CDC_TransmitReady_FS() == 0U)
    {
        return;
    }
...
/* USB CDC 接收回调：按行收集命令，完整后交给主循环解析。 */
