/*
 * 应用层控制调度实现。
 * 主循环维护传感器、标定和通信；1 kHz 串联脉冲诊断、位置环、速度环；
 * 4 kHz 专用于外部 STEP 位置引用。20 kHz 电流环由 ADC 回调独立执行。
 */
#include "app.h"
...
/* 推进“对齐 -> 编码器标定 -> 启动产品态脉冲接口”的一次性上电状态机。 */
    ...
    switch (s_auto_calibration_state)
    {
        ...
        case APP_AUTO_CALIBRATION_ENABLE_TABLE:
            if (ElectricalAngle_EnableNonlinearityCalibration(1U) != 0U)
            {
                ...
                {
                    /* 产品脉冲模式就绪后自动输出JustFloat末脉冲验证帧。 */
                    (void)Debug_StartPulseVofa();
...
/* 初始化全部应用模块；PWM 保持安全制动，等待对齐流程显式启动。 */
void App_Init(void)
{
...
/* 主循环后台：刷新传感器快照、推进标定状态机和非阻塞 USB 调试。 */
void App_Loop(void)
{
    Encoder_Update();
...
/* 4 kHz 外部 STEP 专用位置引用节拍，必须比 1 kHz 速度环更快。 */
