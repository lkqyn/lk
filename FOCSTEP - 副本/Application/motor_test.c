...
#define MOTOR_TEST_DQ_LIVE_HALF_PERIOD_MS       (0x3FFFFFFFU)
#define MOTOR_TEST_DQ_LIVE_DURATION_MS          (0x7FFFFFFEU)
...
/* 从采集到的 d/q 阶跃轨迹提取延迟、上升时间、超调和稳态指标。 */
...
/* 返回整数绝对值，供统计误差和保护阈值计算使用。 */
static int32_t MotorTest_Absolute(int32_t value)
{
    return (value < 0L) ? -value : value;
}

/* 计算无浮点依赖的整数平方根，用于电流 RMS 统计。 */
static uint32_t MotorTest_IntegerSquareRoot(uint64_t value)
{
...
/* 启动连续 d/q 电流周期阶跃，供 VOFA 等实时观察工具使用。 */
                               ...
                               uint32_t duration_ms)
{
    ...
    s_dq_cycle.live_hold = 0U;
    s_dq_cycle.axis_is_q = axis_is_q;
...
/* 启动持续保持；采样仍进入已有的实时无锁 VOFA 缓冲区。 */
uint8_t MotorTest_StartDqLive(int32_t target_d_ma, int32_t target_q_ma)
{
    if (MotorTest_StartDqCycle(target_d_ma,
                               target_q_ma,
                               MOTOR_TEST_DQ_LIVE_HALF_PERIOD_MS,
                               MOTOR_TEST_DQ_LIVE_DURATION_MS) == 0U)
    {
        return 0U;
    }

    /* 周期测试的默认起点为零电流；保持模式立即写入首个用户给定。 */
    s_dq_cycle.live_hold = 1U;
    s_dq_cycle.target_is_high = 1U;
    if (DqCurrentLoop_SetTargets(target_d_ma, target_q_ma) == 0U)
    {
...
/* 在线更新同一轴目标，不允许借此绕开启动时建立的保护配置。 */
uint8_t MotorTest_SetDqLiveTarget(int32_t target_ma)
{
    if ((s_dq_cycle.running == 0U) || (s_dq_cycle.live_hold == 0U) ||
        (MotorTest_Absolute(target_ma) > MOTOR_TEST_DQ_TARGET_LIMIT_MA) ||
        ((s_dq_cycle.axis_is_q != 0U) &&
         (MotorTest_Absolute(target_ma) > MOTOR_TEST_IQ_TARGET_MA)))
    {
        return 0U;
    }

    if (DqCurrentLoop_SetTargets(
            (s_dq_cycle.axis_is_q != 0U) ? 0L : target_ma,
            (s_dq_cycle.axis_is_q != 0U) ? target_ma : 0L) == 0U)
    {
        return 0U;
    }
    if (s_dq_cycle.axis_is_q != 0U)
    {
        s_dq_cycle.target_q_ma = target_ma;
    }
    else
    {
        s_dq_cycle.target_d_ma = target_ma;
    }
    return 1U;
}

uint8_t MotorTest_IsDqLive(void)
{
    return (uint8_t)((s_dq_cycle.running != 0U) &&
                     (s_dq_cycle.live_hold != 0U));
}

uint8_t MotorTest_IsDqLiveQAxis(void)
{
    return s_dq_cycle.axis_is_q;
}

/* 由主循环调用，按设定周期翻转连续 d/q 阶跃目标。 */
