...
/* 返回饱和绝对值，供电流/角度峰值与保护判断。 */
/* 返回整数绝对值，供电流和电压保护比较使用。 */
static int32_t DqCurrentLoop_Absolute(int32_t value)
{
    return (value < 0L) ? -value : value;
}

/* 校验 d/q 目标矢量是否在配置的电流圆内。 */
/* 检查 d/q 目标是否同时满足单轴与合成电流限制。 */
...
/* 将配置的采样到 PWM 更新延迟换算为电角预测计数。 */
/* 按转速和设定延迟计算电角提前量，补偿采样/控制延时。 */
...
/* ADC 20 kHz 回调：采样 A/B 相、变换到 d/q、PI、反变换并更新 PWM。 */
/* 20 kHz 采样回调：执行 Clarke/Park、PI、电压限幅和 PWM 更新。 */
...
/* 校验电角和采样条件后启动同步采样与 d/q 电流闭环。 */
/* 校验配置后启动 d/q 电流闭环，并注册 20 kHz 采样回调。 */
...
/* 原子修改 d/q 电流目标，不修改电压前馈。 */
/* 在线更新 d/q 目标电流；目标不合法时保持原命令。 */
...
/* 原子提交 Id、Iq 与两轴电压前馈，供速度环统一更新工作点。 */
/* 原子更新 d/q 目标、前馈和限压工作点。 */
...
/* 停止同步采样、撤销电流回调并令 PWM 回到安全制动。 */
/* 停止 d/q 电流环、清空输出并注销采样回调。 */
...
/* 在电流环停止时登记/清除逐样本 RAM 观测回调。 */
/* 注册或清除诊断采样观察者；观察者运行在电流环上下文。 */
