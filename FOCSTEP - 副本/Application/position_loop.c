/*
 * 位置外环实现：常规梯形轨迹、外部 STEP 的 CmdPos 引用，以及直接转矩试验。
 * 该模块只产生下游速度/转矩请求，不直接驱动 PWM。
 */
...
/* 4kHz直接转矩通道：每250us最多改变80mA，完整2.2A转换约7ms。 */
#define POSITION_LOOP_DIRECT_TORQUE_SLEW_MA_PER_TICK (80L)
#define POSITION_LOOP_DIRECT_SPEED_FILTER_DIVISOR     (4L)
...
typedef struct
{
    ...
    uint8_t external_direct_torque_mode;
    int32_t external_direct_torque_command_ma;
    /* 外部STEP速度与实际速度之差的低通状态，单位mrpm。 */
    int32_t external_direct_filtered_speed_error_mrpm;
...
void PositionLoop_Init(void)
{
    ...
    s_position_loop.external_direct_torque_mode = 0U;
    s_position_loop.external_direct_torque_command_ma = 0L;
    s_position_loop.external_direct_filtered_speed_error_mrpm = 0L;
                           ...
                           int32_t target_position_count)
{
    ...
    if ((config == 0) || (config->kp_mrpm_per_count <= 0L) ||
        ...
        (config->external_torque_kp_ma_per_count <= 0L) ||
        (config->external_torque_kd_ma_per_rpm < 0L) ||
        (config->external_torque_limit_ma <= 0L) ||
        (config->external_torque_limit_ma >
...
/* 接收 4 kHz PulseInput 提交的 CmdPos、CmdSpeed 和脉冲流状态。 */
...
/* 开关外部位置误差直达 Iq 的试验分支，正常产品轨迹不应依赖该分支。 */
uint8_t PositionLoop_SetExternalDirectTorqueMode(uint8_t enabled)
{
    ...
    interrupt_mask = __get_PRIMASK();
    __disable_irq();
    s_position_loop.external_direct_torque_mode = (enabled != 0U) ? 1U : 0U;
    if (enabled != 0U)
    {
        s_position_loop.external_direct_torque_command_ma = 0L;
        s_position_loop.external_direct_filtered_speed_error_mrpm = 0L;
...
/*
 * 位置控制核心：可选推进内部轨迹或使用外部引用，计算位置 PD 后向速度环
 * 提交速度目标；直接转矩模式则向速度环的受保护 Iq 通道提交请求。
 */
static uint8_t PositionLoop_UpdateControl(uint8_t update_trajectory)
{
    ...
    int64_t direct_torque_ma;
    int32_t direct_speed_error_mrpm;
    int32_t direct_speed_reference_mrpm;
    int32_t torque_delta_ma;
    ...
    {
        /*
         * STEP数量就是唯一的位置目标。该分支不产生SpeedRef，不使用
         * CmdPos，也没有末脉冲超时后的速度交接；位置误差直接成为转矩。
         */
        /*
         * 直接位置弹簧若只用-Kd*实际速度，持续运动必须先积累位置
         * 误差才能产生拉力。这正是末脉冲前落后约123count的根因。
         * 这里改为对“脉冲速度-实际速度”阻尼：频率帮助跟随过程，
         * 但它不是独立速度命令，位置仍是唯一硬约束。
         * STEP流结束后速度参考立即为0，因而绝不会用旧速度继续推过终点。
         */
        direct_speed_reference_mrpm =
            (s_position_loop.external_pulse_stream_active != 0U) ?
            s_position_loop.state.reference_speed_mrpm : 0L;
        s_position_loop.external_direct_filtered_speed_error_mrpm +=
            ((direct_speed_reference_mrpm - measured_speed_mrpm) -
             s_position_loop.external_direct_filtered_speed_error_mrpm) /
            POSITION_LOOP_DIRECT_SPEED_FILTER_DIVISOR;
        direct_speed_error_mrpm =
            s_position_loop.external_direct_filtered_speed_error_mrpm;
        direct_torque_ma =
            (int64_t)s_position_loop.config.external_torque_kp_ma_per_count *
            s_position_loop.state.position_error_count;
        direct_torque_ma +=
            ((int64_t)s_position_loop.config.external_torque_kd_ma_per_rpm *
             direct_speed_error_mrpm) / 1000LL;

        /* 静止且已在一个编码器count内时不来回换向；越出窗口立即恢复刚度。 */
        if ((PositionLoop_Absolute(s_position_loop.state.position_error_count) <=
             1L) &&
            (PositionLoop_Absolute(measured_speed_mrpm) <= 6000L))
        {
            direct_torque_ma = 0LL;
        }
        direct_torque_ma = PositionLoop_Clamp(direct_torque_ma,
            s_position_loop.config.external_torque_limit_ma);

        /*
         * 位置误差可在一个STEP边沿突变5count；Iq必须平滑接近目标，
         * 禁止把这一离散量直接变成2.2A阶跃，避免噪声和母线骤降。
         */
        torque_delta_ma = (int32_t)direct_torque_ma -
            s_position_loop.external_direct_torque_command_ma;
        if (torque_delta_ma > POSITION_LOOP_DIRECT_TORQUE_SLEW_MA_PER_TICK)
        {
            torque_delta_ma = POSITION_LOOP_DIRECT_TORQUE_SLEW_MA_PER_TICK;
        }
        else if (torque_delta_ma <
                 -POSITION_LOOP_DIRECT_TORQUE_SLEW_MA_PER_TICK)
        {
            torque_delta_ma = -POSITION_LOOP_DIRECT_TORQUE_SLEW_MA_PER_TICK;
        }
        s_position_loop.external_direct_torque_command_ma += torque_delta_ma;

        /* 速度值仅供弱磁规划和诊断使用，不再通过速度PI决定Iq。 */
        ...
        (void)SpeedLoop_SetExternalBrakingCurrentLimitMa(
            s_position_loop.config.external_torque_limit_ma);
        (void)SpeedLoop_SetExternalTorqueOverride(
            s_position_loop.external_direct_torque_command_ma, 1U);
        (void)SpeedLoop_ApplyExternalTorqueOverrideFast();
        return 1U;
    }
    if (external_position_mode != 0U)
    {
        ...
        /*
         * 对位置误差的导数做阻尼：d(CmdPos-PosFb)/dt约等于
         * CmdSpeed-SpeedFb。匀速跟随正确时该项为零，不再人为要求
         * 位置P项长期补偿-Kd*SpeedFb；CmdSpeed归零后自然退化为制动项。
         */
        speed_damping_mrpm =
            ((int64_t)damping_gain_mrpm_per_rpm *
             ((int64_t)measured_speed_mrpm -
              s_position_loop.state.reference_speed_mrpm)) / 1000LL;
        speed_command_mrpm =
...
/* 外部 STEP 模式的 4 kHz 快速位置更新，不推进常规梯形轨迹。 */
