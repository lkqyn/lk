...
/* 脉冲模式实时诊断：时间、序号、位置/速度/电流及母线电压，共21路。 */
#define DEBUG_VOFA_PULSE_CHANNEL_COUNT (21U)
#define DEBUG_VOFA_PULSE_FRAME_SIZE \
    ((DEBUG_VOFA_PULSE_CHANNEL_COUNT + 1U) * sizeof(float))

#if (PULSE_INPUT_CONTROL_MODE == PULSE_INPUT_CONTROL_MODE_MICROSTEP_CURRENT)
#define DEBUG_PULSE_MODE_NAME "microstep_current"
#else
#define DEBUG_PULSE_MODE_NAME "closed_loop_power_angle"
#endif
...
static void Debug_VofaBuildPulseFrame(uint8_t *destination)
{
    ...
    const volatile SpeedLoop_State_t *speed = SpeedLoop_GetState();
    const volatile DqCurrentLoop_State_t *current = DqCurrentLoop_GetState();
    const volatile PhaseCurrentLoop_State_t *phase_current =
        PhaseCurrentLoop_GetState();
    const CurrentSense_State_t *phase_measurement = CurrentSense_GetState();
        ...
        PULSE_INPUT_CONTROL_MODE_MICROSTEP_CURRENT)
    {
        ...
        Debug_VofaWriteFloat(&destination[3U * sizeof(float)],
                             (float)Encoder_GetPositionCountFast());
        Debug_VofaWriteFloat(&destination[4U * sizeof(float)],
                             (float)(pulse->reference_position_count -
    ...
    {
        Debug_VofaWriteFloat(&destination[10U * sizeof(float)],
                             (float)phase_current->target_a_ma / 1000.0f);
        Debug_VofaWriteFloat(&destination[11U * sizeof(float)],
                             (float)phase_current->target_b_ma / 1000.0f);
        Debug_VofaWriteFloat(&destination[12U * sizeof(float)],
                             (float)phase_measurement->current_a_ma / 1000.0f);
        Debug_VofaWriteFloat(&destination[13U * sizeof(float)],
                             (float)phase_measurement->current_b_ma / 1000.0f);
        Debug_VofaWriteFloat(&destination[14U * sizeof(float)],
                             (float)phase_current->output_a_mv / 1000.0f);
        Debug_VofaWriteFloat(&destination[15U * sizeof(float)],
                             (float)phase_current->output_b_mv / 1000.0f);
        Debug_VofaWriteFloat(&destination[16U * sizeof(float)],
                             (float)phase_current->saturated_sample_count);
        Debug_VofaWriteFloat(&destination[17U * sizeof(float)],
                             (float)phase_current->fault);
    }
    else
    {
        Debug_VofaWriteFloat(&destination[10U * sizeof(float)],
    ...
    if (power->bus_voltage_mv > s_vofa.pulse_peak_bus_voltage_mv)
    {
        s_vofa.pulse_peak_bus_voltage_mv = power->bus_voltage_mv;
    }
    Debug_VofaWriteFloat(&destination[19U * sizeof(float)],
                         (float)power->bus_voltage_mv / 1000.0f);
    Debug_VofaWriteFloat(&destination[20U * sizeof(float)],
                         (float)s_vofa.pulse_peak_bus_voltage_mv / 1000.0f);
