	...
	//case 0x5013://后退速度
        if(data > 20000)
        {
			...
			else if(address == 0x5013 )
			{
				ADDR_5000_H[13] = 0x4e;
				ADDR_5000_L[13] = 0x20;
				STORAGE_saveSingalParameter(STORAGE_HCD2SPEED_ADDR,data);
			}
            rtl = 0;
        }
        else if(data < 100)
...
//    }
    ...
    {
        GUI_sendWordIsolate(SINGAL_LABLE1_ADDR,0,1);
        GUI_sendWordIsolate(SINGAL_LABLE2_ADDR,0,0);
    }
    else
    {
        GUI_sendWordIsolate(SINGAL_LABLE1_ADDR,0,0);
        GUI_sendWordIsolate(SINGAL_LABLE2_ADDR,0,1);
    }
	...
	GUI_sendWordIsolate(MOTOR3DIR_ADDR, 0, systemPara.MotorUpShouDir);
	GUI_sendWordIsolate(MOTOR4DIR_ADDR, 0, systemPara.MotorDownShouDir);
}
