...
void MotionTrap2_Init(motion_trap2_t *m)
{
    ...
    m->done_window_cnt = 1;             // 小于2count算完成  误差小于0.0005圈
                        ...
                        float dt_s)
{
    ...
    if (((m->dir > 0) && (pos_err_cnt <= 0)) ||
        ((m->dir < 0) && (pos_err_cnt >= 0)))
    {
        mt_finalize_done(m);
        return;
    }
...
void MotionTrap2_TrackToAbsCount(motion_trap2_t *m,
                                 encoder_t *enc,
                                 int64_t abs_target_cnt,
                                 float capture_kp_rpm_per_turn,
                                 float capture_max_rpm,
                                 int32_t done_window_cnt)
{
    int64_t delta_cnt;
    ...
    delta_cnt = abs_target_cnt - enc->pos_cnt_total;
    ...
    m->capture_kp_rpm_per_turn = capture_kp_rpm_per_turn;
    m->capture_max_rpm = capture_max_rpm;
    if (done_window_cnt > 0)
        m->done_window_cnt = done_window_cnt;
