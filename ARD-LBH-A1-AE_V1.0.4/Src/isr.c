...
void EXTI2_IRQHandler(void)
{
    ...
    if(systemPara.status == STATUS_ONLINE)
	{
    	if(systemPara.isWorkTaskRun == 1 && systemPara.RunStatus == 0)
		{
    		...
    		{	//送料停止时
//				if(systemPara.sensorChosen == 1){//下光纤模式
//					systemPara.HCOnceTriggerByUIorIO = 1;
//				}
//				else {//上光纤模式
//					  //if(systemPara.givenOnceOK == 1)//用于飞达剥料互锁
//					  {
//						  //GUI_mainMessageDispIsolate("2", 1);
...
void EXTI3_IRQHandler(void)
{
    ...
    if(systemPara.status == STATUS_ONLINE)
	{
		...
		{
//  		if(systemPara.sensorChosen == 1){//下光纤模式
//  			if(systemPara.isAutoSongEnable == 0 && systemPara.isRSTAutoSongEnable == 0)//非自动模式
//    			systemPara.givenOnceTriggerByUIorIO = 1;
//    		}
//    		else {//上光纤模式
//    			//if(systemPara.givenOnceOK == 0)//用于飞达送料互锁
//    			{
//    				//GUI_mainMessageDispIsolate("1", 1);
...
void SysTick_Handler(void)
{
	...
	static uint32_t logout_count , logout_count0;
		...
		if(++logout_count >= 1000)//1000
		{
			...
			if(++logout_count1 >= 120)//120S
			{
				...
				else if(systemPara.Language == 1)
				{
					...
					GUI_mainMessageDisp("Auto exits successfully.", 24);
				}
