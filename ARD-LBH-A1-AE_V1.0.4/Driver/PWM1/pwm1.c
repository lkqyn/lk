...
static void MX_TIM1_Init(void)
{
    ...
    // ARR ：自动重装载寄存器的值
    // CLK_cnt：计数器的时钟，等于 Fck_int / (psc+1) = 72M/(psc+1)  //200k
    // PWM 信号的周期 T = ARR * (1/CLK_cnt) = ARR*(PSC+1) / 72M  //
    // 占空比P=CCR/(ARR+1)

    //1圈走20MM
    //伺服设置齿轮比4000，
    //20MM/s速度需要4000/s,100mm/s速度需要20000/s  200mm/s=40000/s

    //伺服设置齿轮比2000，
    //20MM/s速度需要2000/s,100mm/s速度需要10000/s  200mm/s=20000/s
    htim1.Instance = TIM1;
    htim1.Init.Prescaler = 36-1;	//原始359  //设置用来作为TIMx时钟频率除数的预分频值
    htim1.Init.CounterMode = TIM_COUNTERMODE_UP;
    htim1.Init.Period = 9999;///9999  //设置在下一个更新事件装入活动的自动重装载寄存器周期的值
    htim1.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;	//TIM_CLOCKDIVISION_DIV1
