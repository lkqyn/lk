...
//        {{GPIOC,GPIO_PIN_9},{GPIOC,GPIO_PIN_6},{GPIOC,GPIO_PIN_8},{GPIOC,GPIO_PIN_7}},
...
void LV8731V_config(void)
{
    ...
    for(i=0;i<4;i++)
    {
        ...
        HAL_GPIO_WritePin(motor[i].M_EN.GPIOx, motor[i].M_EN.pin, GPIO_PIN_RESET);
    }
    ...
    for(i=0;i<4;i++)
    {
        gpio.Pin = motor[i].M_MD1.pin;
        HAL_GPIO_Init(motor[i].M_MD1.GPIOx, &gpio);
    }

    for(i=0;i<4;i++)
    {
        gpio.Pin = motor[i].M_MD2.pin;
        HAL_GPIO_Init(motor[i].M_MD2.GPIOx, &gpio);
    }
}
